**Pyrotechnic Ignition Energy Model**

Overview

This repository presents a physics-based, energy-driven pyrotechnic ignition timing model, validated using MATLAB Simulink / Simscape and mapped directly to an analytical equation suitable for firmware or web-based calculation.

The objective is to replace fixed, heuristic firing delays with a defensible ignition-time computation based on electrical and physical parameters such as battery voltage, internal resistance, wiring resistance, and igniter characteristics.

This project is intended for educational and exploratory purposes, and to demonstrate correct modeling abstractions for short-duration, high-current events.

⸻

Core Principle

Pyrotechnic igniters are energy-driven, not time- or current-driven.

Ignition occurs when sufficient thermal energy is deposited in the igniter bridgewire:

$$E(t) = \int_0^t I^2(t)\,R_{\text{ign}}\,dt$$

Instead of assuming a fixed delay, this model:
	•	computes the minimum required firing time analytically,
	•	applies a conservative safety factor, and
	•	enforces a hard time cutoff.

⸻

Ignition Time Equation

Assuming a short-duration pulse and approximately constant current:

$$t_{\min}=\frac{
E_{\text{req}} \cdot R_{\text{total}}^2
}{
V_{\text{oc}}^2 \cdot R_{\text{ign}}
}
$$

Where:
	•	E_req — required ignition energy
	•	R_ign — igniter resistance
	•	V_oc — battery open-circuit voltage
	•	R_total = R_ign + R_wire + R_bat + R_mos

Final commanded firing time:

$$t_{\text{fire}} = \text{Safety Factor} \times t_{\min}$$

This equation is used consistently across:
	•	documentation,
	•	firmware logic, and
	•	simulation validation.

⸻

Battery Modeling Approach

For short (millisecond-scale) ignition events, the battery is modeled as:
	•	an ideal voltage source V_oc
	•	with a series internal resistance R_bat

Internal resistance is derived conservatively from capacity and C-rating:

$$R_{\text{bat}} \approx \frac{V_{\text{oc}}}{C_r \cdot Q}$$

This avoids unnecessary state-of-charge dynamics while remaining physically realistic for high-current pulses.

⸻

Simulink / Simscape Model

The Simulink model is used to validate, not generate, the ignition timing law.

Model characteristics
	•	Continuous Simscape electrical network
	•	NMOS low-side switching
	•	Energy computed via I²R integration
	•	Static t_fire enforced via Simulink control logic
	•	No runtime parameter mutation
	•	No intrinsic current limiting modeled

The model demonstrates correct abstraction selection and clear separation between:
	•	physical modeling (Simscape)
	•	control logic (Simulink)

⸻

Repository Structure

pyro-energy-ignition-model/
│
├── simulink/
│   ├── pyro_model_basic.slx
│   └── README.md
│
├── docs/
│   ├── theory.md
│   ├── assumptions.md
│   └── limitations.md
│
├── firmware/
│   └── ignition_time_calc.c
│
├── images/
│   ├── block_diagram.png
│   └── energy_plot.png
│
└── README.md


⸻

Firmware Mapping

The ignition timing equation is directly applicable in embedded firmware:
	•	compute t_fire from measured or bounded parameters,
	•	arm a timer for the computed duration,
	•	drive the ignition MOSFET gate,
	•	enforce a hard cutoff.

No lookup tables or empirical delays are required.

⸻

Assumptions and Limitations

Key assumptions are documented explicitly in docs/assumptions.md￼, including:
	•	igniter modeled as fixed resistance,
	•	battery modeled as voltage source plus internal resistance,
	•	no post-ignition electrical behavior modeled,
	•	no intrinsic current limiting,
	•	single-shot ignition only.

Model limitations and non-goals are documented in docs/limitations.md￼.

⸻

**Disclaimer**


>This project is intended for educational and exploratory purposes only.It is not certified, qualified, or validated for flight or safety-critical systems.Users are responsible for ensuring safe implementation in real hardware.

⸻

Motivation

This repository exists to demonstrate:
	•	correct physical abstraction,
	•	energy-based reasoning,
	•	Simulink / Simscape competence,
	•	clean mapping from physics to firmware logic.

It is not intended to replace professional avionics design processes.

